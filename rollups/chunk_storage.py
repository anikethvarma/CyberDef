...
class ChunkStorage:
    ...
    def store_chunks(self, file_id: str, chunks: List[BehavioralChunk]) -> None:
        ...
        try:
            ...
            # Auto-cleanup: Prevent unbounded disk growth by pruning files older than retention limit
            settings = get_settings()
            if settings.enable_rollup_cleanup:
                retention_days = settings.rollup_retention_days
                cleaned = self.cleanup_old_chunks(retention_days)
                if cleaned > 0:
                    logger.info(f"Auto-cleanup removed {cleaned} expired chunk files (retention: {retention_days} days)")
