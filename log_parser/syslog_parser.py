...
class SyslogApacheParser(BaseParser):
        ...
        try:
            ...
            for row in sample_rows[:10]:
                try:
                    ...
                    logger.warning(f"can_parse: failed to check sample row: {e}", exc_info=True)
            ...
            logger.error(f"can_parse failed: {e}", exc_info=True)
            return 0.0
    ...
    def parse_row(self, raw_row: RawEventRow) -> ParsedEvent:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Extended pattern match failed for row {raw_row.row_hash}: {e}", exc_info=True)
            ...
            try:
                ...
                logger.warning(f"Standard pattern match failed for row {raw_row.row_hash}: {e}", exc_info=True)
            ...
            try:
                ...
                logger.warning(f"Flexible pattern match failed for row {raw_row.row_hash}: {e}", exc_info=True)
            ...
            try:
                ...
                logger.error(f"Legacy fallback parsing failed for row {raw_row.row_hash}: {e}", exc_info=True)
                return ParsedEvent(file_id=raw_row.file_id, row_hash=raw_row.row_hash)

        except Exception as e:
            logger.error(f"parse_row critically failed for row {raw_row.row_hash}: {e}", exc_info=True)
            return ParsedEvent(file_id=raw_row.file_id, row_hash=raw_row.row_hash)
    ...
    def _build_from_extended(self, raw_row: RawEventRow, raw: str, m: re.Match[str]) -> ParsedEvent:
        ...
        try:
            ...
            logger.error(f"_build_from_extended failed for row {raw_row.row_hash}: {e}", exc_info=True)
            return ParsedEvent(file_id=raw_row.file_id, row_hash=raw_row.row_hash)
    ...
    def _build_from_standard(self, raw_row: RawEventRow, raw: str, m: re.Match[str]) -> ParsedEvent:
        ...
        try:
            ...
            logger.error(f"_build_from_standard failed for row {raw_row.row_hash}: {e}", exc_info=True)
            return ParsedEvent(file_id=raw_row.file_id, row_hash=raw_row.row_hash)
    ...
    def _build_from_flexible(self, raw_row: RawEventRow, raw: str, m: re.Match[str]) -> ParsedEvent:
        ...
        try:
            ...
            logger.error(f"_build_from_flexible failed for row {raw_row.row_hash}: {e}", exc_info=True)
            return ParsedEvent(file_id=raw_row.file_id, row_hash=raw_row.row_hash)
    ...
    def _parse_flexible_legacy(self, body: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error(f"_parse_flexible_legacy failed: {e}", exc_info=True)
            return None
    ...
    def _get_raw(self, data: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            logger.error(f"_get_raw failed: {e}", exc_info=True)
            return None
    ...
    def _clean_ip(self, value: Any) -> str | None:
        ...
        try:
            ...
            logger.warning(f"_clean_ip failed for value '{value}': {e}", exc_info=True)
            return "-"
    ...
    def _parse_ts(self, ts_str: str | None) -> datetime | None:
        ...
        try:
            ...
            logger.warning(f"_parse_ts failed for ts_str '{ts_str}': {e}", exc_info=True)
            return None
    ...
    def _parse_port(self, value: Any) -> int | None:
        try:
            ...
            logger.warning(f"_parse_port failed for value '{value}': {e}", exc_info=True)
            return None
    ...
    def _parse_int(self, value: Any) -> int | None:
        try:
            ...
            logger.warning(f"_parse_int failed for value '{value}': {e}", exc_info=True)
            return None
